...
void MyGlob(const char * Pattern , void (*FileFuncParm)(const char * FileName))
{
    ...
    strcpy_s(PatCopy, _MAX_PATH*2+1, Pattern);
...
DoRecursion:
    ...
    strncpy_s(BasePattern, 260, PatCopy, BaseEnd);
    BasePattern[BaseEnd] = 0;

    strncpy_s(MatchPattern, 260, PatCopy, PatternEnd);
        ...
        for (a=0;a<NumHave;a++){
            ...
            if (FileList[a].attrib & _A_SUBDIR){
                if (MatchDirs){
                    ...
                    strncat_s(CombinedName, _MAX_PATH*2+1, PatCopy+PatternEnd, _MAX_PATH*2-strlen(CombinedName));
    ...
    if(RecurseAt >= 0){
        strcpy_s(MatchPattern, 260, PatCopy+RecurseAt);
        PatCopy[RecurseAt] = 0;
        strncpy_s(PatCopy+RecurseAt, 521, "*\\**\\", _MAX_PATH*2-RecurseAt);
        strncat_s(PatCopy, 521, MatchPattern, _MAX_PATH*2-strlen(PatCopy));
