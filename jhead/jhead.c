...
static int FileEditComment(char * TempFileName, char * Comment, int CommentSize)
{
...

    {
        ...
        sprintf_s(QuotedPath, 270, "%s \"%s\"",Editor, TempFileName);
        a = system(QuotedPath);
    }
...
static void DoCommand(const char * FileName, int ShowIt)
{
    ...
    strcpy_s(TempName+a, 270, "XXXXXX");
...
static void RelativeName(char * OutFileName, const char * NamePattern, const char * OrigName)
{
    ...
    if (Subst){
        strncpy_s(OutFileName, PATH_MAX, NamePattern, Subst-NamePattern);
        OutFileName[Subst-NamePattern] = 0;
        strncat_s(OutFileName, PATH_MAX, OrigName, PATH_MAX);
        strncat_s(OutFileName, PATH_MAX, Subst+2, PATH_MAX);
    }else{
        strncpy_s(OutFileName, PATH_MAX, NamePattern, PATH_MAX); 
    }
}
...
void RenameAssociated(const char * FileName, char * NewBaseName)
{
    ...
    for (;;){
        ...
        strncpy_s(FilePattern+PathLen, PATH_MAX-PathLen, finddata.name, PATH_MAX-PathLen); // full name with path

        strcpy_s(NewName, PATH_MAX, NewBaseName);
        for(a = strlen(finddata.name);finddata.name[a] != '.';){
            if (--a == 0) goto next_file;
        }
        strncat_s(NewName, PATH_MAX, finddata.name+a, _MAX_PATH-strlen(NewName)); // add extension to new name
...
static void DoFileRenaming(const char * FileName)
{
    ...
    if (!Exif2tm(&tm, ImageInfo.DateTime)){
		...
		localtime_s(&tm, &ImageInfo.FileDateTime);
    }
    

    strncpy_s(NewBaseName, PATH_MAX*2, FileName, PATH_MAX); // Get path component of name.
    ...
    if (strftime_args){
        ...
        strncpy_s(pattern, PATH_MAX+20, strftime_args, PATH_MAX-1);
            ...
            for (a=0;pattern[a];a++){
                if (pattern[a] == '%'){
                    ...
                    if (ppos >= 0 && a<ppos+4){
                        ...
                        pat[a-ppos+1] = '\0';
                        sprintf_s(num, 16, pat, FileSequence); // let printf do the number formatting.
                        nl = strlen(num);
        ...
        // My favourite scheme.
        sprintf_s(NewName, 262, "%02d%02d-%02d%02d%02d",
    ...
    for (a=0;;a++){
        ...
        sprintf_s(NewName, PATH_MAX+10, "%s%s.jpg", NewBaseName, NameExtra);
        ...
        if (stat(NewName, &dummy)){
            ...
            if (rename(FileName, NewName) == 0){
                ...
                if (RenameAssociatedFiles){
                    sprintf_s(NewName, PATH_MAX+10, "%s%s", NewBaseName, NameExtra);
...
static int DoAutoRotate(const char * FileName)
{
    if (ImageInfo.Orientation >= 2 && ImageInfo.Orientation <= 8){
        ...
        if (!ZeroRotateTagOnly){
            ...
            sprintf_s(RotateCommand, PATH_MAX*2+50, "jpegtran -trim -%s -outfile &o &i", Argument);
            ...
            if (ImageInfo.ThumbnailOffset && 
                ...
                strcpy_s(ThumbTempName_in, PATH_MAX+5, FileName);
                strcat_s(ThumbTempName_in, PATH_MAX+5, ".thi");
                strcpy_s(ThumbTempName_out, PATH_MAX+5, FileName);
                strcat_s(ThumbTempName_out, PATH_MAX+5, ".tho");
                SaveThumbnail(ThumbTempName_in);
                sprintf_s(RotateCommand, PATH_MAX*2+50, "jpegtran -trim -%s -outfile \"%s\" \"%s\"",
...
static int RegenerateThumbnail(const char * FileName)
{
    ...
    sprintf_s(ThumbnailGenCommand, PATH_MAX*2+50, "mogrify -thumbnail %dx%d \"%s\"", 
...
void FileTimeAsString(char * TimeStr)
{
	...
	localtime_s(&ts, &ImageInfo.FileDateTime);
...
static void ProcessFile(const char * FileName)
{
    ...
    strncpy_s(ImageInfo.FileName, 261, FileName, PATH_MAX);
    ...
    if (
        ...
        if (CommentInsertfileName){
			...
			fopen_s(&CommentFile, CommentFileName, "r");
            if (CommentFile == NULL){
            ...
            strncpy_s(Comment, MAX_COMMENT_SIZE+1, CommentInsertLiteral, MAX_COMMENT_SIZE);
...
#endif
            {
                ...
                strcpy_s(EditFileName, PATH_MAX+5, FileName);
                strcat_s(EditFileName, PATH_MAX+5, ".txt");
    ...
    if (CommentSavefileName){
        ...
        if (CommentSec != NULL){
			...
			fopen_s(&CommentFile, OutFileName, "w");
       ...
       if (ImageInfo.numDateTimeTags){
            ...
            if (ExifTimeSet){
                ...
                if (DateSetChars){
                    ...
                    a = 1970;
                    sscanf_s(DateSet, "%d", &a);
                    if (a < 1970){
                        strcpy_s(TempBuf, 50, ImageInfo.DateTime);
                        goto skip_unixtime;
                    }
                }
            ...
            sprintf_s(TempBuf, 50, "%04d:%02d:%02d %02d:%02d:%02d",
...
skip_unixtime:         
    ...
    if (Modified){
        ...
        strcpy_s(BackupName, PATH_MAX+5, FileName);
        strcat_s(BackupName, PATH_MAX+5, ".t");
...
//--------------------------------------------------------------------------
// Returns a copy of the EXIF information.
//--------------------------------------------------------------------------
ImageInfo_t GetExifWithJhead(const char * FileName)
{
    int Modified = FALSE;
    ReadMode_t ReadMode;
	ImageInfo_t ExifData;
...
static time_t ParseCmdDate(char * DateSpecified)
{
    ...
    a = sscanf_s(DateSpecified, "%d:%d:%d/%d:%d:%d",
...
int main (int argc, char **argv)
{
    ...
    // Thumbnail manipulation options
            ...
            RegenThumbnail = 160;
            sscanf_s(arg+4, "%d", &RegenThumbnail);
    ...
    // Date/Time manipulation options
            ...
            n = sscanf_s(arg+4, "%d:%d:%d", &hours, &minutes, &seconds);
            ...
            strcpy_s(DateSet, 11, "0000:01:01");
            for (a=0;arg[a+3];a++){
