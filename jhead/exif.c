        ...
        unsigned ExifLength, int NestingLevel)
{
    ...
    for (de=0;de<NumDirEntries;de++){
        ...
        switch(Tag){
            ...
            case TAG_MAKE:
                strncpy_s(ImageInfo.CameraMake, 32, (char *)ValuePtr, ByteCount < 31 ? ByteCount : 31);
                break;

            case TAG_MODEL:
                strncpy_s(ImageInfo.CameraModel, 40, (char *)ValuePtr, ByteCount < 39 ? ByteCount : 39);
                break;

            case TAG_DATETIME_ORIGINAL:
                // If we get a DATETIME_ORIGINAL, we use that one.
                strncpy_s(ImageInfo.DateTime, 20, (char *)ValuePtr, 19);
                // Fallthru...
                ...
                // Copy the comment
                {
                    ...
                    if (msiz > 5 && memcmp(ValuePtr, "ASCII",5) == 0){
                        for (a=5;a<10 && a < msiz;a++){
                            int c = (ValuePtr)[a];
                            if (c != '\0' && c != ' '){
                                strncpy_s(ImageInfo.Comments, 2000, (char *)ValuePtr+a, msiz-a);
                                break;
                            }
                        }
                    }else{
                        strncpy_s(ImageInfo.Comments, 2000, (char *)ValuePtr, msiz);
                    }
                }
                break;
...
int Exif2tm(struct tm * timeptr, char * ExifTime)
{
    ...
    timeptr->tm_sec = 0;
	a = sscanf_s(ExifTime, "%d%*c%d%*c%d%*c%d:%d:%d",
